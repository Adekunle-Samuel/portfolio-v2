...
interface ImageGridProps {
  ...
  largeVideo?: { asset?: { url?: string } } | null
...
export default function ImageGrid({ largeImage, largeVideo, smallImages, onImageClick }: ImageGridProps) {
  const largeImageUrl = largeImage ? getOptimizedImageUrl(largeImage, 2360, 1144) : null
  const largeVideoUrl = largeVideo?.asset?.url
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        onClick={() => !largeVideoUrl && largeImageUrl && onImageClick?.(largeImageUrl)}
      >
        {largeVideoUrl ? (
          <video
            src={largeVideoUrl}
